FROM docker.io/library/postgres:18-alpine AS pgvector-builder
...
FROM docker.io/library/postgres:18-alpine AS runtime
...
RUN apk add --no-cache pgbackrest restic rclone python3 apprise tzdata musl-locales py3-psycopg2 && \
